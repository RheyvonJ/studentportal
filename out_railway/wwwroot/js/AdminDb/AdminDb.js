...
function setupProfessorSubjectSelect() {
    ...
    async function populateMyAssignedSubjects() {
        ...
        // Filter dropdown to *current* school year only (no dropdown UI).
        const computeCurrentSchoolYearLabel = () => {
            const now = new Date();
            const y = now.getFullYear();
            // Match schedule's "current school year" label
            return `${y}-${y + 1}`;
        };
        ...
        const effectiveSY = allSchoolYears.includes(currentSY) ? currentSY : (allSchoolYears[0] || '');

        // Clear school year until a subject/assignment is chosen (but we know what we will filter by)
        ...
        const rebuildOptions = () => {
            ...
            subjects.forEach(s => {
                ...
                if (!code) return;
                if (effectiveSY && schoolYear && schoolYear !== effectiveSY) return;
