document.addEventListener('DOMContentLoaded', () => {
    ...
    const attachEbookBtn = document.getElementById('attachEbookBtn');
    ...
    // ------------------ EBOOK ATTACHMENT (LIBRARY) ------------
    const ebookBackdrop = document.getElementById('amEbookBackdrop');
    const ebookModal = document.getElementById('amEbookModal');
    const ebookSearch = document.getElementById('amEbookSearch');
    const ebookSearchBtn = document.getElementById('amEbookSearchBtn');
    const ebookCancel = document.getElementById('amEbookCancel');
    const ebookResults = document.getElementById('amEbookResults');

    const showEbookModal = () => {
        if (!ebookBackdrop || !ebookModal) return;
        ebookBackdrop.hidden = false;
        ebookModal.hidden = false;
        ebookBackdrop.setAttribute('aria-hidden', 'false');
        try { ebookSearch?.focus(); } catch { }
        // Auto-load all eBooks immediately (no manual search required)
        void runEbookSearch(true);
    };

    const hideEbookModal = () => {
        if (!ebookBackdrop || !ebookModal) return;
        ebookBackdrop.hidden = true;
        ebookModal.hidden = true;
        ebookBackdrop.setAttribute('aria-hidden', 'true');
    };

    attachEbookBtn?.addEventListener('click', (e) => {
        e.preventDefault();
        if (!isEditing) startEditing();
        showEbookModal();
    });
    ebookCancel?.addEventListener('click', hideEbookModal);
    ebookBackdrop?.addEventListener('click', hideEbookModal);

    async function runEbookSearch(loadAll = false) {
        if (!ebookResults) return;
        const q = (ebookSearch?.value || '').trim();
        const effective = loadAll ? '' : q;
        ebookResults.innerHTML = `<div style="padding: 14px; color: rgba(11,33,58,0.7);">${effective ? 'Searching…' : 'Loading all available eBooks…'}</div>`;
        try {
            ...
            if (books.length === 0) {
                ebookResults.innerHTML = '<div style="padding: 14px; color: rgba(11,33,58,0.7);">No eBooks found.</div>';
                return;
            }

            const rows = books.map(b => {
                const title = escapeHtmlForMeta(b.title || '(Untitled)');
                const author = escapeHtmlForMeta(b.author || '');
                const subject = escapeHtmlForMeta(b.subject || '');
                const desc = escapeHtmlForMeta(b.description || '');
                const id = escapeHtmlForMeta(b.id || '');
                return `
                    <div class="aae-library-ebook-row" style="border-radius:0; border-left:none; border-right:none;">
                        <div class="aae-library-ebook-text">
                            <div class="aae-library-ebook-title">${title}</div>
                            <div class="aae-library-ebook-meta">${author}${subject ? ' · ' + subject : ''}${desc ? ' · ' + desc : ''}</div>
                        </div>
                        <button type="button" class="aae-library-ebook-btn am-ebook-select" data-bookid="${id}" style="border:none; cursor:pointer;">Select</button>
                    </div>`;
            }).join('');

            ebookResults.innerHTML = rows;
            ebookResults.querySelectorAll('.am-ebook-select').forEach(btn => {
                btn.addEventListener('click', async () => {
                    const bookId = btn.dataset.bookid;
                    if (!bookId) return;
                    await attachEbookToMaterial(bookId);
                });
            });
        } catch (err) {
            console.error(err);
            ebookResults.innerHTML = '<div style="padding: 14px; color: rgba(11,33,58,0.7);">Failed to search library.</div>';
        }
    }

    async function attachEbookToMaterial(bookId) {
        try {
            ...
            const response = await fetch('/AdminMaterial/SetLinkedEbook', {
                method: 'POST',
                headers: { 'Content-Type': 'application/json' },
                body: JSON.stringify({ materialId, bookId })
            });
            const result = await response.json();
            if (!response.ok || !result?.success) {
                showToast(result?.message || 'Failed to attach eBook', 'error');
                return;
            }

            showToast('✅ eBook attached');
            hideEbookModal();
            // simplest + consistent: refresh to re-render linked eBooks section
            setTimeout(() => window.location.reload(), 350);
        } catch (err) {
            console.error(err);
            showToast('❌ Failed to attach eBook', 'error');
        }
    }

    ebookSearchBtn?.addEventListener('click', runEbookSearch);
    ebookSearch?.addEventListener('keydown', (e) => {
        if (e.key === 'Enter') {
            e.preventDefault();
            runEbookSearch(false);
