    ...
    public class MongoDbService
    {
        ...
        public async Task<List<string>> GetStudentEmailsBySectionIdAsync(string sectionId)
        {
            ...
            try
            {
                ...
                {
                    Builders<BsonDocument>.Filter.Eq("CurrentSectionId", sid),
                    Builders<BsonDocument>.Filter.Eq("currentSectionId", sid)
                };
                // SHSSections._id and SHSSchedules.SectionID may be stored as ObjectId or string
                if (ObjectId.TryParse(sid, out var sidOid))
                {
                    sectionFilters.Add(Builders<BsonDocument>.Filter.Eq("CurrentSectionId", sidOid));
                    sectionFilters.Add(Builders<BsonDocument>.Filter.Eq("currentSectionId", sidOid));
                }
                ...
                if (string.IsNullOrWhiteSpace(sectionId))
                {
                    ...
                    foreach (var collName in meetingCollections)
                    {
                        try
                        {
                            ...
                            if (ObjectId.TryParse(scheduleId.Trim(), out var schedOid))
                            {
