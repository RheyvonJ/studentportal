function setupAdminBottomBar() {
    ...
    menuCircle?.addEventListener('click', () => {
        if (typeof window.navigateWithProfessorLoading === 'function') {
            const dashMsg = typeof window.resolveAdminNavToastMessage === 'function'
                ? window.resolveAdminNavToastMessage('/professordb/ProfessorDb')
                : 'Redirecting to Dashboard';
            window.navigateWithProfessorLoading('/professordb/ProfessorDb', dashMsg, 600);
        } else {
            window.showToast('Redirecting to Dashboard');
        ...
        action.addEventListener('click', () => {
            ...
            if (icon.classList.contains('fa-house')) {
                const dashMsg = typeof window.resolveAdminNavToastMessage === 'function'
                    ? window.resolveAdminNavToastMessage('/professordb/ProfessorDb')
                    : 'Redirecting to Dashboard';
                navigateWithAnimation('/professordb/ProfessorDb', dashMsg);
