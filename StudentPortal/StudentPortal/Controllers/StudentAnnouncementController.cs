...
namespace StudentPortal.Controllers
{
    public class StudentAnnouncementController : Controller
    {
        ...
        public async Task<IActionResult> Index(string classCode, string contentId)
        {
            ...
            var userEmail = (email ?? string.Empty).Trim();
            var ownerEmail = (classItem.OwnerEmail ?? string.Empty).Trim();
            var canManage = !string.IsNullOrWhiteSpace(userEmail)
                && !string.IsNullOrWhiteSpace(ownerEmail)
                && string.Equals(ownerEmail, userEmail, StringComparison.OrdinalIgnoreCase);
            ...
            var vm = new StudentAnnouncementViewModel
            {
                ...
                EditedDate = contentItem.UpdatedAt > contentItem.CreatedAt ? contentItem.UpdatedAt.ToLocalTime().ToString("MMM d, yyyy") : null,
                Files = files,
                RecentAnnouncements = recent,
                CanManageAnnouncement = canManage
            };
