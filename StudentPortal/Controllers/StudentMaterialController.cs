...
namespace StudentPortal.Controllers
{
    public class StudentMaterialController : Controller
    {
        ...
        public StudentMaterialController(MongoDbService mongoDb, IWebHostEnvironment env, LibraryService libraryService)
        {
            _mongoDb = mongoDb;
            _env = env;
        ...
        /// <summary>First visit: link SLSHS Library eBooks from the shared catalog when none are stored yet.</summary>
        private async Task TryAutoLinkLibraryEbooksAsync(StudentPortal.Models.AdminMaterial.ContentItem contentItem, ClassItem classItem)
        {
            try
            {
                ...
                contentItem.UpdatedAt = System.DateTime.UtcNow;
                await _mongoDb.UpdateContentAsync(contentItem);
            }
            catch (System.Exception ex)
            {
                System.Console.WriteLine($"[StudentMaterial] TryAutoLinkLibraryEbooksAsync: {ex.Message}");
            }
        }
    }
}
