...
namespace StudentPortal.Controllers
{
    public class AdminTaskController : Controller
    {
        ...
        public async Task<IActionResult> Index(string classCode, string contentId)
        {
            ...
            // recent tasks (limit to 3 most recently created/uploaded)
            var recentTasks = (await _mongoDb.GetContentsForClassAsync(classItem.Id, classItem.ClassCode))?
                .Where(c => c.Type == "task")
            ...
            var vm = new AdminTaskViewModel
            {
                ...
                EditedDate = contentItem.UpdatedAt,
                Submissions = vmSubmissions,
                TaskMaxGrade = contentItem.MaxGrade,
                RecentMaterials = recentTasks
            };
