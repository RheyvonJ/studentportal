...
function selectBook(id) {
    ...
    if (detailAuthor) detailAuthor.textContent = book.author ? ('Author: ' + book.author) : '';
...
function filterBooks(q) { loadBooks(q || ''); }
...
reserveBtn && reserveBtn.addEventListener('click', function () { if (!selectedBookId) return; reserveBackdrop && (reserveBackdrop.hidden = false); });
reserveYesBtn && reserveYesBtn.addEventListener('click', async function () {
    if (reserveBackdrop) reserveBackdrop.hidden = true;
    if (!selectedBookId) return;
    try {
        const res = await fetch('/Library/ReserveBook', { method: 'POST', headers: { 'Content-Type': 'application/x-www-form-urlencoded' }, body: 'bookId=' + encodeURIComponent(selectedBookId), credentials: 'same-origin' });
        const data = await res.json();
        showToast(data && data.message ? data.message : 'Reservation request sent.');
        if (data && data.success && reserveBtn) reserveBtn.disabled = true;
    } catch { showToast('Error reserving book. Please try again.'); }
});
reserveNoBtn && reserveNoBtn.addEventListener('click', function () { if (reserveBackdrop) reserveBackdrop.hidden = true; });
